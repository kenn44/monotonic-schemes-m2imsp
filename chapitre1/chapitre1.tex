...
On appelle \textbf{splitting de Lie} ou méthode de pas fractionnaires les deux methodes (L1) et (L2). Pour une equation scalaire, ces deux methodes sont identiques et reviennent au meme que de traiter l’equation en une seule fois.
Pour exprimer le splitting dans notre exemple, nous avons ete obliges d’introduire une variable intermediaire $y$. Ceci s’avererait a l’usage peu pratique pour des contextes plus compliques. Nous introduisons donc les semi-groupes d’evolution.
\begin{definition}
	semi groupe Khalil
\end{definition}
...
\begin{pro}
	Cette méthode est qu’elle conserve la norme $L^2$ de la fonction d’onde $\psi$ au cours du temps.
\end{pro}
\begin{ proof }
...
Une exception existe avec le schéma implicite de Cranck-Nicholson défini dans notre cas de la manière suivante :
